        ...
        <div class="flex flex-col space-y-3 md:flex-row md:justify-between md:items-center md:space-y-0 mb-6 text-xs">
            <div class="relative w-full md:w-1/3">
                <input
                    ...
                    <svg class="w-5 h-5 text-gray-400 flex-none absolute right-3 top-1.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M21.71,20.29,18,16.61A9,9,0,1,0,16.61,18l3.68,3.68a1,1,0,0,0,1.42,0A1,1,0,0,0,21.71,20.29ZM11,18a7,7,0,1,1,7-7A7,7,0,0,1,11,18Z"/></svg>
            </div>

            <div class="flex flex-col space-y-3 md:flex-row md:items-center md:space-x-2 md:space-y-0">
                <select v-model="email_verified" class="p-2 px-4 border rounded-lg focus:outline-none focus:border-blue-400 text-gray-600">
            ...
            <div class="overflow-x-auto pb-8">
                <table class="w-full">
                    <tr class="bg-gray-100 text-gray-600 text-xs border-b border-gray-200">
                        ...
                        <!-- Cognome -->
                        <th @click="sortBy('last_name')" class="font-normal text-left uppercase p-3">
                        ...
                        <!-- Email -->
                        <th @click="sortBy('email')" class="font-normal text-left uppercase p-3">
                            <div class="flex space-x-1 items-center cursor-pointer max-w-max">
                                <span>{{ $t('email') }}</span>
                    ...
                    <tr v-for="(user, index) in users.data" :key="user.id" class="odd:bg-gray-50 text-xs text-gray-500 p-3">
                        <td class="p-3 font-semibold text-blue-400 whitespace-nowrap">{{ user.id }}</td>
                        <td class="p-3 whitespace-nowrap">{{ user.first_name }}</td>
                        <td class="p-3 whitespace-nowrap">{{ user.last_name }}</td>
                ...
                <footer v-if="users.meta"  class="mt-8 flex justify-between items-center text-gray-500">

                    <span class="text-xs block">{{ $t('results') }} {{ users.meta.total }}</span>
