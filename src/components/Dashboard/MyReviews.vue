                ...
                <header class="flex flex-col space-y-2 xs:space-y-0 xs:flex-row xs:justify-between xs:items-baseline pb-8 text-sm text-gray-400">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <select v-model="sort" class="border rounded-lg focus:outline-none px-2 py-1.5 cursor-pointer">
                            <option selected value="date.desc">Più recenti</option>
                ...
                <div v-for="review in reviews" :key="review.id" class="flex space-x-4 mb-4 odd:bg-gray-100 rounded-lg p-4">
                    ...
                    <div>
                        ...
                        <div class="mt-4 text-xs text-gray-400">
                            <span>Recensione di: <router-link class="cursor-pointer" :to="{ name: 'product.show', params: { slug: review.product.slug }}">{{ review.product.name }}</router-link></span>
                        </div>
                    </div>
                </div>
            </div>
            <div v-else>
                <p class="text-gray-400 text-center text-sm">Le tue recensioni verranno visualizzati qui.</p>
            </div>
        </section>
    </div>
</template>
