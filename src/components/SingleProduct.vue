<template>
    <div class="relative">
        <router-link :to="{ name: 'product.show', params: { id: product.id }}">
            <div class="bg-gray-200 h-48 w-48 md:h-56 md:w-56 xl:w-full rounded-lg shadow-md"></div>
        </router-link>

        <div class="absolute left-4 top-4">
            <h3 class="text-c-dark-gray">{{ product.name }}</h3>
            <p class="text-c-light-gray text-xs">Prezzo al kg</p>
            <p class="text-sm text-c-dark-gray font-semibold mt-1"><span class="text-c-red">€</span>{{ product.price }}</p>
        </div>
